...
의사 클래스(`:hover`, `:focus-visible`), 의사 요소(`::after`), BEM 수정자(`&--modifier`), 미디어쿼리(`@media`)는 논리적으로 같은 블록에 속하므로 중첩 깊이 계산에서 제외한다.
...
```scss
// _button.scss — 버튼 컴포넌트 스타일 (기본, 링크, 아이콘)
```
