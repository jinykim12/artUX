...
| 페이지 유형 | 전달 방식 | 정확도 | 이유 |
|------------|-----------|--------|------|
| **랜딩페이지** (비주얼 중심, 자유 레이아웃) | **스크린샷 파일** (전체 캡처 이미지) | ★★★★★ | 자유 배치가 많아 전체 화면을 눈으로 보는 게 섹션 구분과 비율 파악에 유리 |
| **서브페이지** (게시판, 폼, 관리자) | **피그마 URL** | ★★★★★ | 반복되는 컴포넌트 구조를 API로 정확히 읽음 |
| **인터랙션** (탭, 모달, 상태변화) | **피그마 URL** | ★★★★★ | 상태별 화면을 API가 구분해서 읽음 |
| **이미지/일러스트 많은 페이지** | **스크린샷 파일** | ★★★★ | 시각적 비율, 배치를 이미지로 보는 게 정확 |

**정리:**
- **랜딩페이지, 비주얼 페이지** → 스크린샷 파일 (전체 캡처)
- **서브페이지, 기능 페이지** → 피그마 URL
- **둘 다 있으면** 가장 정확

### 스크린샷 파일 전달 시 주의

- 전체 페이지를 **세로로 길게 한 장**으로 캡처 (섹션별로 자르지 않음)
- 가능하면 **PC + 모바일** 2장
- 파일 형식: PNG 또는 JPG (PDF도 가능)
